...
static void ssd1306_init(void)
{
    ...
    ssd1306_write_cmd(0x80); // Default Setting for Display Clock Divide Ratio and Oscillator 
    ...
    ssd1306_write_cmd(0x12); // Alternative com pin config, disable com left/right remap
 ...
 */
static int alm_oled_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	ssd1306_init();
	ssd1306_fill(0xff);
...
static int __init alm_init(void)
{
	...
	if ((alm.client = i2c_new_client_device(alm.adapter, &alm_oled_board)) == NULL)
	{
